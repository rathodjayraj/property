...
use Botble\RealEstate\Forms\SpotlightForm;
use Botble\Base\Forms\FormBuilder;
use Botble\RealEstate\Tables\SpotlightTable;
use DB;

class SpotlightController extends BaseController
{
    /**
     ...
     * @throws \Throwable
     */
    public function index(SpotlightTable $table)
    {
    ...
    {
        page_title()->setTitle(trans('Spotlight.create'));

        return $formBuilder->create(SpotlightForm::class)->renderForm();
    }
     ...
     */
    public function store(SpotlightRequest $request, BaseHttpResponse $response)
    {	
        $testimonial = $this->SpotlightRepository->createOrUpdate($request->input());

        event(new CreatedContentEvent(PACKAGE_MODULE_SCREEN_NAME, $request, $testimonial));

        return $response
            ->setPreviousUrl(route('Spotlight.index'))
            ->setNextUrl(route('Spotlight.edit', $testimonial->id))
