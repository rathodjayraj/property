 ...
 * @return string
 */
function get_re_categories_name($id)
{
    $requirements = DB::select('select name from re_categories where id ='.$id);
...
{
    $citylist = DB::select('select id,name from cities');
	
    return  $citylist;
}
function getfacilitylist()
{
    $facilistlist = DB::select("SELECT t1.*,t2.`reference_id`,t2.`reference_type`,t2.`meta_value` FROM re_features t1 JOIN meta_boxes t2 ON t1.id = t2.reference_id"); 
    return  $facilistlist;
...
{
    $facilistlist = DB::select("SELECT t1.*,t2.`reference_id`,t2.`reference_type`,t2.`meta_value` FROM  re_facilities  t1 JOIN meta_boxes t2 ON t1.id = t2.reference_id"); 
    return  $facilistlist;
...
if (is_plugin_active('real-estate')) {
    ...
    function save_customize_form_fields ($type, $request, $object) {
        if (in_array(get_class($object), [Facility::class, Feature::class , Category::class]) && $request->has('icon_image')) {
